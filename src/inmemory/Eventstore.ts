...
export class Eventstore extends EventEmitter {
  ...
  storeEventAtDatabase(event: EventType) {
    this.database.events.push(event);
  }

  storeSnapshotAtDatabase(snapshot: Snapshot) {
    this.database.snapshots.push(snapshot);
  }

  updateEventInDatabaseAtIndex(index: number, newEventData: EventType) {
  ...
  async getEventStream(aggregateId: AggregateId, options: Options = {}) {
    if (!aggregateId) {
      throw new Error('Aggregate id is missing.');
    }
  ...
  async saveEvents({ events } : { events: EventType[] }) {
    ...
    events.forEach((event) => {
      ...
      event.data = omitByDeep(event.data, (value: any) => value === undefined);
  ...
  async markEventsAsPublished({ aggregateId, fromRevision, toRevision }: any) {
    ...
    const shouldEventBeMarkedAsPublished = (event: EventType) =>
