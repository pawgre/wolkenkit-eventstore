...
import { MysqlEventstore as Eventstore, Event } from '../../../src';
import path = require('path')

describe('Mysql', () => {
  const url =  env.MYSQL_URL;
  const nonExistentUrl = 'mysql://localhost/non-existent';
  ...
  describe('initialize', () => {
    ...
    it('throws an error if url is missing.', async () => {
      await assert.that(async () => {
        await eventstore.initialize(<any>{});
      }).is.throwingAsync('Url is missing.');
    });

    it('throws an error if namespace is missing.', async () => {
      await assert.that(async () => {
        await eventstore.initialize({url, namespace: null});
      }).is.throwingAsync('Namespace is missing.');
    });

    it('returns an error if the database is not reachable.', async () => {
      await assert.that(async () => {
